...
- `?gclid` (Google Ads click ID), `?fbclid` (Facebook click ID), `?etext` (Yandex): force-cache rules added — all stripped from cache key generation while preserving URL parameters intact for the visitor
