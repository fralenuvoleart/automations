...
### wp-login.php Hidden on pbservices.ge
- **Fact:** The `/wp-login.php` path has been hidden (renamed/moved) on the pbservices.ge installation — the default WordPress login URL is not accessible. The actual login URL is at a non-standard path.
- **Source:** User confirmation, 2026-08-10.
- **Skill impact:** When analyzing access/error logs for pbservices.ge, requests to `/wp-login.php` that return 404 are expected — the path does not exist. Do not flag these as brute-force attempts or recommend rate-limiting them. The Kinsta default wp-login.php rate limit (6 req/min) still applies to the path but is effectively irrelevant since the path is hidden. If the actual login URL is ever identified in logs, treat it with the same sensitivity as wp-login.php.
...
- **Fact:** Kinsta only forces cache on specific known-safe query parameters (e.g., `?p=`). Any URL with a query string bypasses page cache by default. Custom server caching rules have been added for `utm_*`, `yclid`, `gclid`, `fbclid`, `etext`, and `eopreload` — these parameters are **stripped/ignored for cache key generation** so the page is served from cache, while the URL parameters remain intact in the browser. The visitor sees their tracking parameters preserved; the cache sees one canonical URL.
- **Source:** Kinsta Support Chat; user confirmation 2026-08-08.
- **Skill impact:** When analyzing cache BYPASS rates correlated with query params, note that the above parameters should NOT cause BYPASSes — if they do, the rule may have been removed or overridden. Any other query parameter still causes a BYPASS by default.
...
## 🕷️ Crawler Artifact Patterns

### WordPress Interactivity Module 404s (`@wordpress/interactivity`)
- **Fact:** 404s for paths like `/wp-includes/js/dist/script-modules/block-library/navigation/@wordpress/interactivity` are caused by **dumb crawlers**, not broken code. WordPress core uses ES module bare specifiers (`import ... from "@wordpress/interactivity"`) that browsers resolve via an HTML `<script type="importmap">` tag. Crawlers scanning raw `.js` files without executing HTML naively append the bare specifier to the script's directory, generating a nonexistent path. The `/wp-includes/` directory is core WordPress — themes and plugins cannot modify its internal import paths.
...
### XML Sitemap Full-Page Caching (24h TTL)
- **Fact:** A custom Nginx caching rule was added to enable full-page caching for all XML sitemap endpoints (`/sitemap.xml`, `/sitemap-*.xml`, language-prefixed variants like `/ru/sitemap.xml`, `/zh/sitemap.xml`, `/ar/sitemap.xml`) with a **24-hour TTL**.
- **Source:** User, 2026-08-08.
- **Skill impact:** When analyzing cache performance, note that XML sitemap requests should show as cache HITs after the first request within each 24h window. If sitemap MISSes appear in the cache-perf log, this may indicate the TTL has expired or the rule was bypassed. Do not recommend adding sitemap caching — it is already in place.
